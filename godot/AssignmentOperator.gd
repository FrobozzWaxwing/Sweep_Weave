...
func get_value(leaf = null):
	...
	if (null == operand_1):
		return null
	var value = 0
	if (operand_1 is ScriptManager or operand_1 is SWScriptElement):
		value = operand_1.get_value(leaf)
	elif (TYPE_INT == typeof(operand_1) or TYPE_REAL == typeof(operand_1)):
		value = operand_1
	else:
		return null
	return value
...
func compile(parent_storyworld, include_editor_only_variables = false):
	if (!(operand_0 is BNumberPointer and operand_1 is ScriptManager)):
		return null
	var result = {}
	result["Set"] = operand_0.compile(parent_storyworld, include_editor_only_variables)
	result["to"] = operand_1.compile(parent_storyworld, include_editor_only_variables)
	return result
...
func load_from_json_v0_0_21(storyworld, data_to_load):
	...
	if (data_to_load.has_all(["Set", "to"])):
		...
		if (data_to_load["to"].has("script_element_type")):
			...
			script.load_from_json_v0_0_21(storyworld, data_to_load["to"])
			operand_1 = script
